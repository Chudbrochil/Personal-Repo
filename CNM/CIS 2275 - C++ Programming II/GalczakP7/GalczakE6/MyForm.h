...
namespace GalczakE6 {
	...
	private: System::Windows::Forms::GroupBox^  grpEncodeType;
	private: System::Windows::Forms::RadioButton^  radQwerty;
	private: System::Windows::Forms::RadioButton^  radShifty;
	private: System::Windows::Forms::RadioButton^  radPrimeShift;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->grpEncodeType = (gcnew System::Windows::Forms::GroupBox());
			this->radQwerty = (gcnew System::Windows::Forms::RadioButton());
			this->radShifty = (gcnew System::Windows::Forms::RadioButton());
			this->radPrimeShift = (gcnew System::Windows::Forms::RadioButton());
			this->grpEncode->SuspendLayout();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->nudKey))->BeginInit();
			this->grpEncodeType->SuspendLayout();
			this->SuspendLayout();
			// 
			// openFileDialog1
			// 
			...
			// grpEncodeType
			// 
			this->grpEncodeType->Controls->Add(this->radQwerty);
			this->grpEncodeType->Controls->Add(this->radShifty);
			this->grpEncodeType->Controls->Add(this->radPrimeShift);
			this->grpEncodeType->Location = System::Drawing::Point(246, 51);
			this->grpEncodeType->Name = L"grpEncodeType";
			this->grpEncodeType->Size = System::Drawing::Size(177, 91);
			this->grpEncodeType->TabIndex = 14;
			this->grpEncodeType->TabStop = false;
			this->grpEncodeType->Text = L"Encode Type:";
			// 
			// radQwerty
			// 
			this->radQwerty->AutoSize = true;
			this->radQwerty->Location = System::Drawing::Point(7, 67);
			this->radQwerty->Name = L"radQwerty";
			this->radQwerty->Size = System::Drawing::Size(134, 17);
			this->radQwerty->TabIndex = 2;
			this->radQwerty->TabStop = true;
			this->radQwerty->Text = L"Qwerty + Base Encode";
			this->radQwerty->UseVisualStyleBackColor = true;
			// 
			// radShifty
			// 
			this->radShifty->AutoSize = true;
			this->radShifty->Location = System::Drawing::Point(7, 43);
			this->radShifty->Name = L"radShifty";
			this->radShifty->Size = System::Drawing::Size(127, 17);
			this->radShifty->TabIndex = 1;
			this->radShifty->TabStop = true;
			this->radShifty->Text = L"Shifty + Base Encode";
			this->radShifty->UseVisualStyleBackColor = true;
			// 
			// radPrimeShift
			// 
			this->radPrimeShift->AutoSize = true;
			this->radPrimeShift->Checked = true;
			this->radPrimeShift->Location = System::Drawing::Point(7, 19);
			this->radPrimeShift->Name = L"radPrimeShift";
			this->radPrimeShift->Size = System::Drawing::Size(151, 17);
			this->radPrimeShift->TabIndex = 0;
			this->radPrimeShift->TabStop = true;
			this->radPrimeShift->Text = L"Prime Shift + Base Encode";
			this->radPrimeShift->UseVisualStyleBackColor = true;
			// 
			// MyForm
			// 
			...
			this->Controls->Add(this->grpEncodeType);
			...
			this->Name = L"MyForm";
			this->Text = L"Anthony Galczak - Program 7 - 3 Inherited Enigma Encodes";
			...
			this->grpEncodeType->ResumeLayout(false);
			this->grpEncodeType->PerformLayout();
...
#pragma endregion
	...
	{
		//Make three managed strings to read in the data from the file
		String^ mess;
		String^ kkey;
		String^ msgIndex;
		...
		// Assigning index of type of enigma algorithm to use
		//int index;
		//if (radPrimeShift->Checked == true) { index = 0; }
		//if (radShifty->Checked == true) { index = 1; }
		//if (radQwerty->Checked == true) { index = 2; }
		...
		if (openFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			if ((openFileDialog1->OpenFile()) != nullptr)  //Check that file is open
			{
				...
				mess = input->ReadLine();
				kkey = input->ReadLine();
				msgIndex = input->ReadLine();
				input->Close();

				// Converting msgIndex managed string to a regular c++ string using to_string, then converting that to an int
				string decodeIndex;
				To_string(msgIndex, decodeIndex);
				int decIndex = atoi(decodeIndex.c_str());
				...
				pE[decIndex]->SetCodedMessage(codedMess, kk);

				//and display the results in your textboxes: message, key and filename
				txtMess->Text = gcnew String(pE[decIndex]->GetDecodedMessage().c_str());
				txtCodedMessage->Text = mess;
				txtKKey->Text = pE[decIndex]->GetKey().ToString();
	...
	private: System::Void btnEncode_Click(System::Object^  sender, System::EventArgs^  e) 
	{
		...
		// Checking to see which radbutton is checked to set the index
		int index;
		if (radPrimeShift->Checked == true) { index = 0; }
		if (radShifty->Checked == true) { index = 1; }
		if (radQwerty->Checked == true) { index = 2; }
		...
		if (saveFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			...
			if (rbEnterKey->Checked)
			{
				...
				pE[index]->SetMessage(mess, kk);
			}
			else
			{
				pE[index]->SetMessage(mess);
			}
			//now get back the result
			txtMess->Text = gcnew String(pE[index]->GetDecodedMessage().c_str());
			txtCodedMessage->Text = gcnew String(pE[index]->GetCodedMessage().c_str());
			txtKKey->Text = pE[index]->GetKey().ToString();
