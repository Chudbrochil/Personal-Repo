...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        encoding = self.encoder(pack_padded_sequence(X, source_lengths))

        # Construct enc_hidden of size (b, src_len, h*2)
        enc_hiddens = pad_packed_sequence(encoding[0], batch_first=True)[0]

        # Construct last_hidden of size (b, h*2))
        last_hidden, last_cell = encoding[1]
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            # Put o_prev after Y_t_squeezed to fix "low BLEU score" from Piazza @598.
            Ybar_t = torch.cat((Y_t_squeezed, o_prev), dim = 1)
            step = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            dec_state = step[0]
            o_t = step[1]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # added dims to avoid error of "implicit dims deprecated"
