...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        # Shift
        if transition == "S":
            word_to_add = self.buffer.pop(0)
            self.stack.append(word_to_add)

        elif transition == "LA":
            right_word = self.stack.pop(-1)
            left_word = self.stack.pop(-1)

            self.dependencies.append((right_word, left_word))
            ...
            right_word = self.stack.pop(-1)
            left_word = self.stack.pop(-1)
            self.dependencies.append((left_word, right_word))
            self.stack.append(left_word)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Add all the dependencies in the order of partial parses
