...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #self.hidden_to_logits_weight = nn.Parameter(torch.Tensor(self.hidden_size * self.embed_size, n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        # Select the indices in the embeddings from our vectorized w
        x = torch.index_select(self.embeddings, 0, torch.flatten(w))

        # Re-shape the tensor to the correct dimensions
    ...
    def forward(self, w):
        ...
        # Lookup the embeddings in our custom nn.Embedding function.
        x = self.embedding_lookup(w) # [4, 36] -> [4,1080]
        h = nn.functional.relu_(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias) # [4,1080] * [1080,200] + [1,200] -> [4, 200]

        h = self.dropout(h)

        logits = torch.matmul(h, self.hidden_to_logits_weight) + self.hidden_to_logits_bias # [4, 200] * [200, 3] + [1,4] -> [4,3]
        ### END YOUR CODE
        return logits
