...
def run_hypothesis_only_evaluation():
    ...
    for condition in ['word_disjoint', 'edge_disjoint']:
        for combo_func in ['vec_concatenate', 'hypothesis_only']:
            experiment = nli.wordentail_experiment(
                ...
                vector_func=glove_vec,
                vector_combo_func=eval(combo_func)) # eval() will evaluate this string as a function?
            
            hypothesis_experiments[(condition, combo_func)] = experiment['macro-F1']
...
class TorchDeepNeuralClassifier(TorchShallowNeuralClassifier):
    ...
    def define_graph(self):
        ...
        h_1 = nn.Linear(self.input_dim, self.hidden_dim)
        d_1 = nn.Dropout(self.dropout_prob)
        h_3 = nn.Linear(self.hidden_dim, self.n_classes_)
        
        
        return nn.Sequential(
            h_1,
            d_1,
            self.hidden_activation, # This is a tanh()
            h_3)
        ##### YOUR CODE HERE
...
##### YOUR CODE HERE
deep_net = TorchDeepNeuralClassifier(hidden_dim=50, max_iter=1000)
word_disjoint_experiment = nli.wordentail_experiment(
    train_data=wordentail_data['word_disjoint']['train'],
    assess_data=wordentail_data['word_disjoint']['dev'], 
    model=deep_net, 
...
# Made a deeper and wider fully connected network with relu instead of tanh().
# Trained on edge_disjoint instead of just word disjoint.
# I'm pretty sure this dev score (0.909) is overfitting to training data.
# Given more time, I would investigate the ROBERTa implementation inside
# pytorch.

# 0.687 - 3layer net, dropout 0.3, hidden_dim 400, max_iter 200
# 0.701/.68x - 3layer net, dropout 0.3, hidden_dim 500, max_iter 250

# train on whole dataset (edge_disjoint) assess on dev
# 0.909 ? - 3layer net, dropout 0.3, hidden_dim 500, max_iter 250

# My peak score was: 0.909
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    ...
    class TorchDeepNeuralClassifier2(TorchShallowNeuralClassifier):
        def __init__(self, dropout_prob=0.7, **kwargs):
            self.dropout_prob = dropout_prob
            super().__init__(**kwargs)
        ...
        def define_graph(self):
            ...
            h_1 = nn.Linear(self.input_dim, self.hidden_dim)
            d_1 = nn.Dropout(self.dropout_prob)
            h_3 = nn.Linear(self.hidden_dim, self.hidden_dim)
            d_2 = nn.Dropout(self.dropout_prob)
            h_4 = nn.Linear(self.hidden_dim, self.n_classes_)
    ...
    # Same neural net, but with ReLU and 0.3 dropout instead of 0.7
    deep_net = TorchDeepNeuralClassifier2(dropout_prob = 0.3, hidden_dim=500, max_iter=250)
    word_disjoint_experiment = nli.wordentail_experiment(
       train_data=wordentail_data['edge_disjoint']['train'],
       assess_data=wordentail_data['word_disjoint']['dev'], 
       model=deep_net, 
       vector_func=glove_vec,
       vector_combo_func=vec_concatenate)
