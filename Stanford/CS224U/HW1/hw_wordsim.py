...
def run_giga_ppmi_baseline():
    ...
    giga20 = pd.read_csv('data/vsmdata/giga_window20-flat.csv.gz', index_col=0)
    giga20_pmi = vsm.pmi(giga20) 
    giga20_similarity_eval = full_word_similarity_evaluation(giga20_pmi)
    
    return giga20_similarity_eval
...
def run_ppmi_lsa_pipeline(count_df, k):
    ...
    df_eval = full_word_similarity_evaluation(df_lsa)
...
def run_small_glove_evals():
    ...
    for it in [10, 100, 200]:
        glv = GloVe(max_iter=it)
        giga_glove = glv.fit(giga20.values)
        giga_df = pd.DataFrame(giga_glove, index=giga20.index)
        giga_eval = full_word_similarity_evaluation(giga_df)
        
        # Need to extract the macro-average value out of the frame that returns.
        glove_dict[it] = giga_eval["Macro-average"]
...
def dice(u, v):
    ...
    # Zip u and v together, take each tuples' min, sum all these elements together.
    # Divide this all by the sum of both lists.
    return 1 - (2 * sum(map(min, zip(u,v)))) / (sum(u) + sum(v))
...
def ttest(df):
    ...
    col_row = np.outer(row_totals, col_totals) / (total * total)
    
    # Divide by total to get Xij
    df = df.div(total)
    # Subtract each column and row sum product elementwise
    df = df.sub(col_row)
    # Divide each sqrt column and row sum product elementwise
    df = df.div(np.sqrt(col_row))
...
def subword_enrichment(df, n=4):
    ...
    for word in df.index:        
        arr.append(vsm.character_level_rep(word, df_ngrams, n))

    char_level_rep = pd.DataFrame(arr, index = df.index)
...
# The below code runs PPMI re-weighting on the giga5 data. It additionally runs the t-test re-weighting on the giga5 data.
# Then, I concatenate the two dataframes together and run an evaluation on the combined frame.

# Previous models
# Baseline (run_small_glove_evals), MA: 0.0353, 0.1528, 0.1920
# Giga20 - Glove - 10000iterations, MA: 0.29902265070528544
# Giga5 - Glove - 3500 iterations, MA: 0.35883054532003467
# Giga5 concat Giga20 - PPMI concat ttest, MA: 0.4615019209207839

# My peak score was: 0.4909199281764295
# This is my code
def original_submission():
    giga5 = pd.read_csv('data/vsmdata/giga_window5-scaled.csv.gz', index_col=0)
    giga5_pmi = vsm.pmi(giga5)
    giga5_ttest = ttest(giga5)
    giga5_add = giga5_pmi + giga5_ttest
    
    return giga5_add
    
if 'IS_GRADESCOPE_ENV' not in os.environ:
    display(full_word_similarity_evaluation(original_submission()))
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    ...
    giga_eval = full_word_similarity_evaluation(giga5_add)

    print(giga_eval["Macro-average"])
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    ...
    giga20 = pd.read_csv('data/vsmdata/giga_window20-flat.csv.gz', index_col=0)
    giga20_pmi = vsm.pmi(giga20)
    giga20_ttest = ttest(giga20)

    giga20_add = giga20_pmi + giga20_ttest

    giga_whole = giga5_add + giga20_add

    giga_eval = full_word_similarity_evaluation(giga_whole)
